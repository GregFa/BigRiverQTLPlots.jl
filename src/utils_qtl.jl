...
"""
plotQTL(multiLODs::Array{Float64, 2}, dfpInfo::DataFrame, dfgInfo::DataFrame;
         thr::Float64 = 5.0, kwargs...)

Generates a scatter plot for eQTL analysis.

## Arguments
- `multiLODs` is the matrix containing the maximum value of LOD score of each phenotype and its corresponding index.
...
- `thr` is the LOD threshold value, default is `5.0``.
...
# rename!(dfgInfo, Dict(Symbol(mbColname) => :geno_mb));
# rename!(dfgInfo, Dict(Symbol(chrColname) => :geno_chr)); 
